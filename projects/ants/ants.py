...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        tile = self.place
        index = 0
        while tile is not beehive:
            if tile.bees and self.min_range <= index <= self.max_range:
                return rANTdom_else_none(tile.bees)
            index += 1
            tile = tile.entrance
        return None
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        amount_is_armor = (amount == self.armor)

        if amount_is_armor and self.true_queen:
            bees_win()
        elif amount_is_armor:
...
def make_slow(action, bee):
    ...
    def slow(gamestate):
        ...
        length -= 1
        if length >= 0:
            every_other_turn = gamestate.time % 2
            if every_other_turn == 0:
