...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    #dont modify expressions
    name = expressions
    if name == nil:
        return None
    #WHILE LOOP eval each part of expression

    while name.rest != nil:
        scheme_eval(name.first, env) #performs the action
        name = name.rest
    
    return scheme_eval(name.first, env) # change this line, performs action, returns value from scheme eval
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        # BEGIN PROBLEM 10
        formalss = len(formals)
        valss = len(vals)
        new_frame = Frame(self)
        if valss != formalss:
            raise SchemeError
        for x in range(formalss):
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        python_args = []
        # BEGIN PROBLEM 3
        args1 = args #dont modify args argument
        while args1 != nil:
            python_args.append(args1.first)
            args1 = args1.rest       
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        eval_shortened = scheme_eval(expressions.rest.first, env)
        env.define(target, eval_shortened)
        return target
        # END PROBLEM 5
