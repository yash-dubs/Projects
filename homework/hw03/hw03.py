...
def composer(func=lambda x: x):
    ...
    def func_adder(g):
        z = func
        return composer(lambda x: z(g(x)))
...
def g_iter(n):
    ...
    if n <=3:
        return n
    while x < n - 3:
        before, middle, after = before + (2 * middle) + (3 * after), before, middle
        x = x + 1
    return before
...
def missing_digits(n):
    ...
    if wo_lasts == 0:
        return 0
    elif last_digit == second_last:
        return 0 + missing_digits(wo_lasts)
    elif last_digit - second_last == 1:
        return 0 + missing_digits(wo_lasts)
    else:
        return missing_digits(wo_lasts) + (last_digit - second_last - 1)
