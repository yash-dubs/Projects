...
def make_withdraw(balance, password):
    ...
    """
    incorrects = []
    def helper(amount, password_attempt):
        nonlocal incorrects
        nonlocal balance
        if len(incorrects) >= 3:
            return "Too many incorrect attempts. Attempts: " + str(incorrects[:4])
        if password_attempt == password:
            ...
            incorrects.append(password_attempt)
